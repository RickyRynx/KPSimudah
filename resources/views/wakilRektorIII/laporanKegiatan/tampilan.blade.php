@extends('layout.masterLaporanKegiatanBidangKemahasiswaan')
@section('content')
    <div class="container">
        <h1>Daftar UKM</h1>
                ...
                <tbody>
                    @forelse ($ukm as $unit)
                        <tr>
                            <td>{{ $unit->nama_ukm }}</td>
                            <td>
                                <a href="{{ route('laporanKegiatanKemahasiswaan.show', ['id' => $unit->id]) }}"
...
@section('scripts')
    <script>
        document.getElementById('ukmForm').addEventListener('submit', function(event) {
            ...
            var formAction = "{{ url('absensiUKM') }}/" + selectedUkmId;
